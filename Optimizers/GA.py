...
class GA():
    ...
    def crossover(self, p1, p2, eta_c):
    ...
    def run(self, exports):
        ...
        # self.rank_pop(pop)
        # c1, c2 = self.crossover(pop[0], pop[1], 2)
        # print(c1.decode_str, c2.decode_str)
        # print(pop[0].decode_str, pop[1].decode_str)
        # sys.exit()
        # self.mutate(pop[0], self.params['mutation_rate'], 2)
        ...
        for i in range(0, self.params['generations']):
            ...
            for idv in pop:
                cal_pop.append(idv.decode_str)

            pop_ranked = self.rank_pop(pop)
            best_fitness = pop[pop_ranked[0]].fitness
            print(f'best_fitness: {best_fitness}')
            print(pop[pop_ranked[0]].decode[0])
            print(pop[pop_ranked[0]].detail[1])

            next_pop = self.select_elite(pop, pop_ranked, self.params['elite_size'])
            while (len(next_pop) < self.params['pop_size']):
                p1, p2 = self.tournament_selection(pop)
                c1, c2 = self.crossover(p1, p2, 1)
                self.mutate(c1, self.params['mutation_rate'], 1.5)
                self.mutate(c2, self.params['mutation_rate'], 1.5)
