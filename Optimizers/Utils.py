...
# return index of list contain element in 2d list
    ...
    for file in filenames:
        if 'stats' in file:
            try:
                ...
                col_num = df.columns.shape[0]-1
                old_data = df.values.tolist()
                old_data = list(map(lambda i: old_data[i][:7] + list(map(lambda x: round(x, 5), old_data[i][7:col_num])), range(len(old_data))))
                ...
                for i in range(len(upper_data)):
                    if upper_data[i][6] == temp_min:
                        cur_min.append(upper_data[i])
                    elif upper_data[i][6] < temp_min:
                        temp_min = upper_data[i][6]
                        cur_min.clear()
                        cur_min.append(upper_data[i])
                    if (i+1) % 5 == 0:
                        print(cur_min)
                        iter_arr = [x[7] for x in cur_min]
                        max_id = iter_arr.index(max(iter_arr))
                        new_data.append(cur_min[max_id])
                        cur_min.clear()
                        temp_min = 99999999999
            except FileNotFoundError:
                print(f"No such file or directory: {output_file}")
    # iter_cols = list(map(lambda x: f'iter{x}', range(col_num-6)))
    df2 = pd.DataFrame(new_data, columns=['Instance', 'version', 'run_time', 'tech_num', 'work_time', 'params', 'cost', 'iter'])
    to_excel(output_file, 'upper', df2)
